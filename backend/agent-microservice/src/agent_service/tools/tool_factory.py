...
from agent_service.prompts.tool_prompt import READING_TEMPLATE, LISTENING_TEMPLATE, TASK_TEMPLATE, WEB_SUMMARY_TEMPLATE, TRANSLATION_TEMPLATE, PHRASING_TEMPLATE, RETRIEVER_TEMPLATE
    ...
    def initialize_tools(self):
        ...
        for config_attr, map_set in self.tool_map.items():
            if getattr(self.config, config_attr, False):
                ...
                llm, prompt_id, max_tokens = config.llm, config.prompt_id, config.max_tokens
                tool_instance = tool_class(
                    name, description, llm, 
                    prompt_id, prompt_template, max_tokens
                    )
