from agent_service.prompts.task_generation_examples import (
    GRAMMAR_GAP_FILLING,
    GRAMMAR_OPEN_ENDED,
...
from agent_service.exceptions.lesson_exceptions import (
    ExtractingExercisesError,
    ExercisesNotGeneratedException,
)
...
class TaskGenerator(Tool):
    ...
    def run(self, input: str):
        ...
        query_single_choice, query_gap_filling, query_open_ended = self.build_prompts(
            parsed_input
        )
        ...
        while areInvalid and gen_attempts != 0:
            try:
                ...
                if not done_single_choice:
                    single_choice_questions = self.generate_exercises(
                        query_single_choice,
                        "single-choice",
                        int(parsed_input["single-choice"]),
                        700,
                    )
                    done_single_choice = True
                if not done_gap_filling:
                    gap_filling_exercises = self.generate_exercises(
                        query_gap_filling,
                        "gap-filling",
                        int(parsed_input["gap-filling"]),
                        700,
                    )
                    done_gap_filling = True
                if not done_open_ended:
                    open_ended_questions = self.generate_exercises(
                        query_open_ended,
                        "open-ended",
                        int(parsed_input["open-ended"]),
                        700,
                    )
        ...
        exercises = (
            single_choice_questions + gap_filling_exercises + open_ended_questions
        )
    ...
    def build_prompts(self, input: Dict[str, str]):
        ...
        num_single_choice, num_gap_filling, num_open_ended = (
            input["single-choice"],
            input["gap-filling"],
            input["open-ended"],
        )
        if (
            not isinstance(num_single_choice, int)
            or not isinstance(num_gap_filling, int)
            or not isinstance(num_open_ended, int)
        ):
        ...
        if input["type"] == "Grammar":
            ...
            examples_single_choice = (
                GRAMMAR_SINGLE_CHOICE
                + self.lesson_retriever.get_examples(
                    topic=main_topic, task_type="single-choice"
                )
            )
            examples_gap_filling = (
                GRAMMAR_GAP_FILLING
                + self.lesson_retriever.get_examples(
                    topic=main_topic, task_type="gap-filling"
                )
            )
            examples_open_ended = GRAMMAR_OPEN_ENDED

            input_single_choice = (
                f"[{main_topic}][{secondary_topic}][{num_single_choice}]"
            )
        ...
        query_single_choice = self.prompt.generate_prompt(
            name_id=self.prompt_id,
            input_format_and_examples=examples_single_choice,
            text=(input_single_choice + f"[{input['text']}]"),
        )
        query_gap_filling = self.prompt.generate_prompt(
            name_id=self.prompt_id,
            input_format_and_examples=examples_gap_filling,
            text=(input_gap_filling + f"[{input['text']}]"),
        )
        query_open_ended = self.prompt.generate_prompt(
            name_id=self.prompt_id,
            input_format_and_examples=examples_open_ended,
            text=(input_open_ended + f"[{input['text']}]"),
        )
    ...
    def generate_exercises(
        self,
        query: str,
        type: Literal["single-choice", "gap-filling", "open-ended"],
        num: int,
        max_tokens: int,
    ):
        """
        ...
        logging.info(
            f"TaskGenerator:generate_exercises: \nQUERY = \n{query}\nTYPE = \n{type}\nNUM = \n{num}\n\n"
        )
        if num == 0:
            return ""
