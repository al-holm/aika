...
class Translator(Tool):
    ...
    def run(self, input: str) -> str:
        ...
        if target_language == "":
            answer = "Du hast das falsche Format als Action Input gegeben.  \
            Bitte benutzte das Action Input Format: der zu übersetzende Text in der Originalsprache - [DE/RU/EN/TR/AR]"
        else:
            input = input.split("- [")[0]
            answer = str(
                self.translator.translate_text(input, target_lang=target_language)
            )
