...
class TestAgent(unittest.TestCase):
    ...
    def test_init_prompts(self, mock_create_prompts):
        ...
        mock_create_prompts.assert_called_once_with(
            {AgentMode.PLAN: ACTION_PROMPT_QA, AgentMode.VAL: VALIDATION_PROMPT_QA}
        )

    @patch("agent_service.prompts.prompt_builder.PromptBuilder.update_prompt")
    @patch(
        "agent_service.agent.agent.Agent.add_trajectory_examples_to_prompts",
    ...
    @patch(
        "agent_service.prompts.prompt_builder.PromptBuilder.generate_prompt",
    ...
    @patch(
        "agent_service.tools.tool_executor.ToolExecutor.execute",
        return_value="Mock Observation",
    )
    def test_execute_step(self, mock_execute):
        """Tests executing a step and updating the reasoning logger."""
        step = AgentStep(
            thought="Test thought", action="Test action", action_input="Test input"
        )
    ...
    def test_validate_step(self, mock_llm_run):
        ...
        with patch.object(
            self.agent, "process_validation_thought", return_value=True
        ) as mock_process_validation_thought:
            is_final = self.agent.validate_step()
            self.assertTrue(is_final)
            mock_llm_run.assert_called_once()
            mock_process_validation_thought.assert_called_once_with(
                "Validation Thought"
            )
