...
class TestLessonMaster(unittest.TestCase):
    ...
    def test_parse_query_correct_input(self):
        ...
        self.assertEqual(
            result["main-topic"],
            "Ich stelle mich vor. Wie alt bin ich? Woher komme ich? Was mache ich beruflich (kurz)? Was mache ich in der Freizeit?",
        )
        self.assertEqual(result["secondary-topic"], "None")
    ...
    def test_create_text_reading(self):
        ...
        matches = re.findall(
            r"\[(.*?)\]", self.lesson_master.task_generator_query, re.DOTALL
        )
